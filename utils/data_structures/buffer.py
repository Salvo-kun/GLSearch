...
class BinBuffer:
    def __init__(self, capacity, sample_strat, biased=None, scalable=False, no_trival_pairs=False):
        ...
        self.no_trival_pairs = no_trival_pairs
    ...
    def extend(self, experiences):
        ...
        n_rm = max(0, len(self.history) + len(experiences) - self.capacity)  # number of elements to rm
    ...
    def sample(self, n_sample):
        ...
        if self.biased == 'bias':
            ...
            while len(bin_keys_samples) < n_sample:
                ...
                bin_keys_samples.extend([bin_key for _ in range(len(self.bins[bin_key]))])
        ...
        for bin_key, n_sample_bin in bin_dict.items():
            ...
            if self.replace:
                experiences_bin = [bin[i] for i in np.random.choice(len(bin), n_sample_bin, replace=True)]
            else:
                experiences_bin = []
                while n_sample_bin > len(bin):
                    experiences_bin.extend(list(bin))
                    n_sample_bin -= len(bin)
                experiences_bin.extend(list(np.random.choice(list(bin), n_sample_bin, replace=False)))
