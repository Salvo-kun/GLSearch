...
class PCAModel(torch.nn.Module):
    def __init__(self, desc, device, n_outputs, tao1=1, tao2=1, sinkhorn_iters=5, sinkhorn_eps=1e-6, aff_max=85):
    ...
    def forward(self, ins, batch_data, model):
        ...
        for pair in pair_list:
            ...
            mne = self._forward_pair(x1, edge1, x2, edge2)
            x1x2t_li.append(mne)
            pair.assign_y_pred_list([mne for _ in range(self.n_outputs)], format='torch_{}'.format(self.device))
        return x1x2t_li
