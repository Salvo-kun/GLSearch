...
class DVN(nn.Module):
    def __init__(self,
        ...
        self.stride_num = Q_mode  # should encapsulate into tunable interact ops
        assert opt.interact_ops is not None
        ...
        dim_dqn_list = _get_dims(dim_dqn_vec, 1)
        ...
        if opt.num_nodes_dqn_max > 0:
            ...
            prune_dim_list = _get_dims(prune_in_dim, 1)
            self.MLP_prune = MLP(prune_in_dim, 1, num_hidden_lyr=len(prune_dim_list), activation_type='elu',
                                 hidden_channels=prune_dim_list, bn=False)

        if self.compute_gs:
            self.MLP_g_big = MLP(n_dim, n_dim, num_hidden_lyr=len(dim_big_list), activation_type='elu',
        ...
        for emb_mode in opt.emb_mode_list:
            if emb_mode == 'gs':
                ...
                if opt.simplified_sg_emb:
                    self.MLP_sg = MLP(n_dim, n_dim, num_hidden_lyr=len(dim_big_list), activation_type='elu',
                                      hidden_channels=dim_big_list, bn=False)
                else:
                    self.MLP_sg = MLP(dim_interact_out, dim_interact_out, num_hidden_lyr=len(dim_small_list),
                                      activation_type='elu', hidden_channels=dim_small_list, bn=False)
            elif emb_mode == 'abds':
                self.MLP_abds_small, self.MLP_abds_big, self.MLP_abd_big = self.get_MLP_bds(n_dim, dim_big_list,
                                                                                            dim_interact_out)
                ...
                self.MLP_ubds_small, self.MLP_ubds_big, self.MLP_ubd_big = self.get_MLP_bds(n_dim, dim_big_list,
                                                                                            dim_interact_out)
                ...
                self.MLP_bbds_small, self.MLP_bbds_big, self.MLP_bbd_big = self.get_MLP_bds(n_dim, dim_big_list,
                                                                                            dim_interact_out)
        ...
        self.MLP_final = MLP(dim_dqn_vec, 1, num_hidden_lyr=len(dim_dqn_list), activation_type='elu',
    ...
    def _init_tunable_interact_weights(self, in_dim: int, final_d: int) -> None:
        ...
        for inter_name in inter_name_list:
            ...
            # create MLPs to move from D to final_d. Each MLP halves the dimensionality
            dim_comb_MLP = _get_dims(D, final_d)
    ...
    def _get_bds(self, x1, x2, all_bidomains, mode, state, gs, q_vec_idx, timer):
        if mode == 'abds':
            ...
            default_vec = self.bbds_default
        else:
            assert False

        if len(all_bidomains) > 0:
            # bds1_list_raw, bds2_list_raw = [], []
...
def _get_dims(in_dim: int, out_dim: int) -> List[int]:
    """
    Get the dimensions of the intermediate MLP layers. Each layer is half the size of the previous layer.
    """
    dims = []
    dim_val = in_dim
