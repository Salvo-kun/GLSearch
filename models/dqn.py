...
class Q_network_v1(nn.Module):
    ...
    def __call__(self, dqn_input: DQNInput, detach_in_chunking_stage=False):
        x1_in, x2_in = self.encode(dqn_input.state.ins_g1), self.encode(dqn_input.state.ins_g2)
        q_vec = _chunked_interact(x1_in, x2_in, dqn_input, detach_in_chunking_stage,
                                  self.dvn)
        return q_vec
...
def _chunked_interact(x1_in: torch.Tensor, x2_in: torch.Tensor, dqn_input: DQNInput,
                      detach_in_chunking_stage: bool, fun: DVN):
    ...
    if num_actions > max_chunk_size:
        dqn_input_chunk_list = get_chunks(dqn_input, max_chunk_size)
        q_vec_chunk_list = []
        ...
        q_vec = merge_chunks(q_vec_chunk_list)
    else:
        q_vec = fun(x1_in, x2_in, dqn_input)
    return q_vec


def get_chunks(dqn_input: DQNInput, max_chunk_size: int) -> List[DQNInput]:
    """Split DQNinput into smaller DQNInput chunks of size max_chunk_size."""
    prev_slice_idx = 0
    chunks = []
    while prev_slice_idx < len(dqn_input.action_space_data.action_space[0]):
        next_slice_idx = prev_slice_idx + max_chunk_size
        chunks.append(obtain_chunk(dqn_input, prev_slice_idx, next_slice_idx))
        prev_slice_idx = next_slice_idx
    return chunks


def obtain_chunk(dqn_input, prev_slice_idx, next_slice_idx):
    ...
    if not opt.scalable:
        ...
        action_space_data_chunk = ActionSpaceDataScalable(
            action_space_chunk,
...
def merge_chunks(x_in_chunk_list):
